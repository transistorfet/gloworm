...
#define MMU_TABLE_ADDRESS_SHORT(desc)			(align_down(((uint32_t) (desc)), PAGE_SIZE))
#define MMU_TABLE_STATUS_SHORT(desc)			(align_remainder(((uint32_t) (desc)), PAGE_SIZE))
...
#define MMU_TABLE_ADDRESS_LONG(desc)			((uint32_t) (align_down(((uint32_t) (desc)), PAGE_SIZE)))
...
#define MMU_FLAG_OP		0x0003
#define MMU_OP_MAP		0x0001		/// Map a range of addresses
#define MMU_OP_UNMAP		0x0002		/// Unmap and free all entries visited

#define MMU_FLAG_TYPE		0x0030
#define MMU_TYPE_UNALLOCATED	0x0000		/// Map all table entries, but leave the descriptors invalid, so a page fault can be caught
#define MMU_TYPE_PREALLOCATED	0x0010		/// Allocate pages for the entire range
#define MMU_TYPE_FIXED		0x0020		/// Map the entire range to the same physical address as the corresponding virtual address

#define MMU_FLAG_COPY_ON_WRITE	0x0200		/// Force the page descriptors to be read-only so that a write will cause a page fault
#define MMU_FLAG_NOCACHE	0x0400		/// Disable hardware caching for the mapped range (eg. for memory-mapped device access)
#define MMU_FLAG_SUPERVISOR	0x0800		/// Set the supervisor flag on mapped pages
